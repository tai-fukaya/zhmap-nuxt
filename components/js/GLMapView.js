...
export default class GLMapView {
  constructor($canvas) {
    ...
    const cameraZpos = (this.height / 2) * Math.sqrt(3)
  ...
  calcurateGeometry() {
    ...
    const dataSize = this.mapData.length
    const longitudeCoeff = this.width / (maxLng - minLng)
    const latitudeCoeff = this.height / (maxLat - minLat)

    const coeff = Math.min(longitudeCoeff, latitudeCoeff)
    const marginLeft = (this.width - (maxLng - minLng) * coeff) / 2
    const marginTop = (this.height - (maxLat - minLat) * coeff) / 2
    ...
    const colors = new Float32Array(dataSize * 3)
    const positions = new Float32Array(dataSize * 3)
    this.mapData.forEach((data, i) => {
      ...
      // 位置の設定
      positions[baseId + 0] = Math.floor(
        marginLeft + (data.longitude - minLng) * coeff
      )
      positions[baseId + 1] = Math.floor(
        marginTop + (data.latitude - minLat) * coeff
      )
